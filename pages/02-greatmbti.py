...
from st_pages import Page, Section, add_page_title, add_indentation
import numpy as np
...
# st-pages 설정
add_page_title() # 앱 제목을 자동으로 추가
add_indentation() # 사이드바의 계층 구조를 추가

# 페이지 정의
st.session_state.pages = [
    Page("app.py", "홈", "🏠"),
    Page("pages/mbti_info.py", "MBTI 정보 탐색", "🧠"),
]
...
if "mbti_explanation" not in st.session_state:
    st.session_state["mbti_explanation"] = MBTI_EXPLANATION
...
def main_page():
    ...
    with col1:
        ...
        if selected_mbti != "선택하세요":
            ...
            이제 **'MBTI 정보 탐색 🧠'** 페이지로 이동해서 상세 정보를 확인해보세요.
            """)
...
[Image of the 16 MBTI personality type icons arranged in a grid with the four dichotomies labelled]
